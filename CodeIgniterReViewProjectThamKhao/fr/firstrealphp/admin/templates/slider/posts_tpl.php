    ...
    function initSwitch() {
        $(".switch-inputx").bootstrapSwitch({ offColor: 'danger', onSwitchChange: function (event, state) {
                ...
                $.ajax({
                    type: "POST",
                    url: "ajax/ajax.php",
                    data: {id: $id, action: $action, post: 'slider_cat',status: status},
                    success: function (data) {
                    }
                })
            }
...
function delete1(id) {
                    ...
                    type: "POST",
                    url: "ajax/ajax.php",
                    data: {id: id,action: "delete",table: "slider_cat"},
                    success: function (data) {
                    if (data==1) { 
                    ...
                    <div class="m-portlet__head-caption" style="position: relative;">
                        <div class="m-portlet__head-title">
                            <h3 class="m-portlet__head-text">
                               SLIDER CAT<small>LIST SLIDER CAT</small>
                            </h3>
                        </div>
                        <a style="position: absolute;right: 20px;top: 12px;" href="index.php?post=slider&action=add" title="New Category" class="btn btn-focus m-btn m-btn--custom m-btn--icon m-btn--air">
                        <span>
...
ajaxload();    
    ...
    function ajaxload() {
        ...
        $("#m_datatable").mDatatable({
            data: {
                type: "remote",
                source: {read: {url:"index.php?post=slider&action=data_lists"}},
                pageSize: 20,
            ...
            columns: [
                ...
                {
                    field: "Add Picture", title: "Add Picture", width: 200,
                    template: function (t) {
                        console.log(t)
                        return ` <span style="width: 110px;">
                    <a href="index.php?post=slider&action=media&slider_id=`+t.ID+`" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill " title="Add Picture"><button type="button" class="btn btn-primary">Add Picture</button></a>                    
                    </span>`
                    }
                },                   
                {
                ...
                },
                {
			        ...
			        <a href="index.php?post=slider&action=edit&id=`+t.ID+`&parent_id=`+t.Parent_ID+`" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill " title="Edit"><i class="la la-edit"></i></a>
