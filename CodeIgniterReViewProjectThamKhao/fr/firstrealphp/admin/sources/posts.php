...
function get_posts() {
    ...
    global $d;
        ...
        $sql = "select * from posts where ID>0 ".$string_s;
...
function get_post() {
    ...
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?post=posts&action=man");
    $sql = "select * from posts where ID='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?post=posts&action=man");
...
function save_post() {
    ...
    if ($id) { 
        ...
        $d->reset();
        $sql_pro="select * from posts where Link='".changeTitle($_POST['name'])."' and ID <> '".$id."' ";
        $d->query($sql_pro);
        ...
        $d->setWhere('ID', $id);
        if ($d->update($data)) {
        transfer("Cập nhật dữ liệu thành công", "index.php?post=posts&action=man&curPage=" . $_REQUEST['curPage'] . "");               
        } else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?post=posts&action=man");
    } else {
        ...
        if ($d->insert($data)) {
            ...
            if($dem > 0 || $dem1 > 0 ){ 
                ...
                $d->setWhere('ID', $id);
                if ($d->update($data)){}
             transfer("Lưu dữ liệu bị lỗi", "index.php?post=posts&action=man");
        } else
            transfer("Lưu dữ liệu bị lỗi", "index.php?post=posts&action=man");
    }
}
function delete_post() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            transfer("Xóa dữ liệu thành công", "index.php?post=posts&action=man");
        else
            transfer("Xóa dữ liệu bị lỗi", "index.php?post=posts&action=man");
    }
}
